...
def movingTheFile(start, end):
    ...
    for posNum in range(start, end):
        ...
        print('Copying {} to {}'.format(og_path, newfilePath))
        shutil.copy(og_path, newfilePath)
...
for i in range(0, listLen + 1, 10):
    start = i
    if (listLen - i) < 10:  # the last ten files won't use threads
        end = i + (listLen-i)
        movingTheFile(start, end)
    else:
        end = i + 9
        downloadThread = threading.Thread(
            target=movingTheFile, args=(start, end))
